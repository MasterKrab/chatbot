...
class ChatManager():
    ...
    def generate(self, prompt: str) -> str:
        ...
        final_chain = (
            ...
            | RunnablePassthrough.assign(output_text = (lambda x: f"CONTEXTO: {x['classification']['type']}"))
            | RunnablePassthrough.assign(context = (lambda x: self.retrievers[x['classification']['type']].retrieve(prompt)))
            | RunnablePassthrough.assign(prompt = (lambda x: prompt_template.render({"context": x['context'], "question": x['question']})))
